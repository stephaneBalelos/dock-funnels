<template>
  <div class="flex flex-col gap-4 py-4">
    <Form
      ...
      @change="validateForm"
      @submit="(e) => onFormSubmit(e as FormSubmitEvent<FormFieldCustomHtml>)"
...
const schema = z.object({
  ...
  html_content: z.string().min(1, "HTML-Inhalt ist erforderlich"),
});

const state = reactive<FormFieldCustomHtml>({
  ...
  html_content: field.value.html_content || "",
});

const errorState = ref<z.ZodError<FormFieldCustomHtml> | null>(null);

function onFormSubmit($event: FormSubmitEvent<FormFieldCustomHtml>) {
