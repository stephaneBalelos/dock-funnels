<template>
  <div v-if="field" :class="
    'flex gap-2 p-2 border rounded cursor-pointer transition-colors duration-200 ' + 
    (editorStore.selectedFieldName.value === field.field_name
          ? 'border-primary-300 bg-primary-50'
          : 'border-gray-200')">
    <div v-if="editorStore.selectedFieldName.value === field.field_name" class="flex flex-col items-center justify-between h-full">
      <Button
        v-if="!editorStore.editorState.value.isLoading && !isFirstInStep"
        v-tooltip="{
        ...
        severity="secondary"
        variant="text"
        @click="editorStore.moveFieldUpRelativeToStep(field.field_name, field.step_index)"
      >
        <Icon icon="heroicons:chevron-double-up-solid" />
      </Button>
      <Button
        v-if="!editorStore.editorState.value.isLoading && !isLastInStep"
        v-tooltip="{
        ...
        variant="text"
        class="mt-auto"
        @click="editorStore.moveFieldDownRelativeToStep(field.field_name, field.step_index)"
      >
        <Icon icon="heroicons:chevron-double-down-solid" />
      </Button>
    </div>
    <div
      :class="
